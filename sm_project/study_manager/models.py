...
class Appointment(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.id:
            log(user=self.user, action='added',  extra={'Date': self.app_date,
                                                        'Time': self.app_time})
        else:
            log(user=self.user, action='updated', extra={'Date': self.app_date,
                                                         'Time': self.app_time})
        super(Appointment, self).save(*args, **kwargs)

    def __unicode__(self):
        return '%s %s' % (self.volunteer_id.fore_names,
                          self.volunteer_id.surname)
